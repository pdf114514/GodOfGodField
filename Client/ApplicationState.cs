...
public class ApplicationState(IJSInProcessRuntime js, NavigationManager navigation) {
    ...
    public string IdToken { get {
        var d = js.LSGetItem("firebase:authUser:AIzaSyCBvMvZkHymK04BfEaERtbmELhyL8-mtAg:[DEFAULT]");
        if (d is null) return string.Empty;
        var j = JsonDocument.Parse(d);
        return j.RootElement.GetProperty("stsTokenManager").GetProperty("accessToken").GetString() ?? string.Empty;
    } }
